            ...
            <div class="row">
                <div class="col-lg-2 col-md-2 col-12">
                    ...
                    <div class="logo">
                        @php
                            $settings=DB::table('settings')->get();
                            $logoPath = Helper::imageUrl(optional($settings->first())->logo ?? null) ?? asset('frontend/img/logo.png');
                        @endphp
                        <a href="{{route('home')}}"><img src="{{ $logoPath }}" alt="logo"></a>
                    </div>
                    <!--/ End Logo -->
                    <!-- Search Form -->
                                    ...
                                    <ul class="shopping-list">
                                        {{-- {{Helper::getAllProductFromCart()}} --}}
                                            @foreach(Helper::getAllProductFromWishlist() as $data)
                                                    @php
                                                        $photoList = array_filter(explode(',',$data->product['photo']));
                                                        $wishlistPhoto = Helper::imageUrl($photoList[0] ?? null) ?? 'https://via.placeholder.com/100x100?text=No+Image';
                                                    @endphp
                                                    <li>
                                                        <a href="{{route('wishlist-delete',$data->id)}}" class="remove" title="Remove this item"><i class="fa fa-remove"></i></a>
                                                        <a class="cart-img" href="#"><img src="{{ $wishlistPhoto }}" alt="{{$data->product['title']}}"></a>
                                    ...
                                    <ul class="shopping-list">
                                        {{-- {{Helper::getAllProductFromCart()}} --}}
                                            @foreach(Helper::getAllProductFromCart() as $data)
                                                    @php
                                                        $photoList = array_filter(explode(',',$data->product['photo']));
                                                        $cartPhoto = Helper::imageUrl($photoList[0] ?? null) ?? 'https://via.placeholder.com/100x100?text=No+Image';
                                                    @endphp
                                                    <li>
                                                        <a href="{{route('cart-delete',$data->id)}}" class="remove" title="Remove this item"><i class="fa fa-remove"></i></a>
                                                        <a class="cart-img" href="#"><img src="{{ $cartPhoto }}" alt="{{$data->product['title']}}"></a>
                                                        <h4><a href="{{route('product-detail',$data->product['slug'])}}" target="_blank">{{$data->product['title']}}</a></h4>
