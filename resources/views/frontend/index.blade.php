...
@section('title','Arvila || Trang Chủ')
...
@if(count($banners)>0)
    <section id="Gslider" class="carousel slide" data-ride="carousel">
        ...
        <div class="carousel-inner" role="listbox">
                @foreach($banners as $key=>$banner)
                @php
                    $bannerPhoto = Helper::imageUrl($banner->photo);
                @endphp
                <div class="carousel-item {{(($key==0)? 'active' : '')}}">
                    <img class="first-slide" src="{{ $bannerPhoto ?? 'https://via.placeholder.com/1900x600?text=Banner' }}" alt="{{ $banner->title }}">
                                    ...
                                    <div class="single-product">
                                        <div class="product-img">
                                            <a href="{{route('product-detail',$product->slug)}}">
                                                @php
                                                    $photoList = array_filter(explode(',',$product->photo));
                                                    $primaryPhoto = Helper::imageUrl($photoList[0] ?? null) ?? 'https://via.placeholder.com/400x400?text=No+Image';
                                                @endphp
                                                <img class="default-img" src="{{ $primaryPhoto }}" alt="{{ $product->title }}">
                                                <img class="hover-img" src="{{ $primaryPhoto }}" alt="{{ $product->title }}">
                                                @if($product->stock<=0)
...
<section class="shop-home-list section">
    <div class="container">
        ...
        $primaryPhoto = Helper::imageUrl($photoList[0] ?? null) ?? 'https://via.placeholder.com/400x400?text=No+Image';
                                        @endphp
                                        <img src="{{ $primaryPhoto }}" alt="{{ $product->title }}">
                ...
                @foreach($posts as $post)
                    <div class="col-lg-4 col-md-6 col-12">
                        ...
                        <div class="shop-single-blog">
                            @php
                                $postImage = Helper::imageUrl($post->photo);
                                if (empty($postImage)) {
                                    $postImage = 'https://via.placeholder.com/600x400?text=Blog';
                                }
                            @endphp
