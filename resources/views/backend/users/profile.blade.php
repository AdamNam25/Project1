...
@php
    ...
    if ($profilePhotoUrl && !preg_match('/^https?:\/\//', $profilePhotoUrl)) {
        $profilePhotoUrl = \Illuminate\Support\Str::startsWith($profilePhotoUrl, '/') ? $profilePhotoUrl : asset($profilePhotoUrl);
    }
@endphp
        ...
        <div class="row">
            <div class="col-md-4">
                <div class="card">
                    <div class="image">
                        @if($profilePhotoUrl)
                        <img class="card-img-top img-fluid roundend-circle mt-4" style="border-radius:50%;height:80px;width:80px;margin:auto;" src="{{$profilePhotoUrl}}" alt="profile picture">
                        @else
                    ...
                    <div class="form-group">
                      ...
                      <div class="custom-file">
                          <input type="file"
                                 ...
                                 data-label-target="#profile-photo-label">
                          <label class="custom-file-label" for="profile-photo-input" id="profile-photo-label">Lựa chọn ảnh</label>
                      </div>
                      <div class="mt-3">
                          <img id="profile-photo-preview"
                               ...
                               style="max-height: 150px; {{ $profilePhoto ? '' : 'display:none;' }}">
                      </div>
                        @error('photo')
                        <span class="text-danger">{{$message}}</span>
                        @enderror
...
<style>
    ...
    .image{
        background:url('{{asset('backend/img/background.jpg')}}?v={{time()}}');
        height:150px;
